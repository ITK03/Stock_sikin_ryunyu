        ...
        <div className="sheet-body">
          ...
          <section>
            ...
            <dl>
              ...
              <dt>急増(初動)</dt>
              <dd>売買代金の増加率(SBI等と同形式)。直近1〜3日の平均売買代金が、過去25営業日平均(平常時)に対して何%増えたかで降順。2日・3日でも上位＝連日で急増が続いている＝初動。極小型株の見かけ倍率はフィルタ(売買代金の下限)で調整。データは定期更新のため、場中の急騰は反映が遅れることがある。場中の更新では当日の売買代金を終日ペースに換算して早期に検知する。基準の25日平均は表示期間の直前まで(例: 1日なら昨日まで)を使うため、数日前から噴いている銘柄は下がり、当日始まった銘柄が上位に来る。</dd>
            </dl>
          </section>
          ...
          <section>
            <h3>マイセクター</h3>
            <p>
              セクタータブの「マイセクター」は、好きな銘柄を自分でレート(重要度)付きで
              束ねられる機能です。<b>レート1〜5の比重は2乗(1/4/9/16/25)</b>で、5が主力・
              1が周辺という重みになります。
            </p>
            <p>
              セクター騰落率は構成銘柄の<b>寄与度の合計</b>です。寄与度は
              「その銘柄の重み×騰落率 ÷ 重みの合計」で、絶対値が大きい銘柄ほど一覧の
              上位に来ます(上昇/下落どちらのセクターでも押し上げ・押し下げの主因が分かります)。
            </p>
            <p className="dim">データは端末内(このブラウザ)にのみ保存されます。「入出力」からJSONで書き出し・読み込みができ、機種変更時などに持ち出せます。</p>
          </section>
